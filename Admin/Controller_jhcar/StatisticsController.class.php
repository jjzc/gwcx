...
class StatisticsController extends CommonController {
    ...
    public function byCarData(){
        ...
        for($i=0;$i<count($res)&& count($res);$i++){
            ...
            //获取出行记录总数
            $cars[$i]["travelSum"]=$travelM->where($mapTravel)->where("car_id=".$res[$i]["id"]." and is_del=0")->count();

            $cars[$i]["countMaintain"]=$travelM->where($mapTravel)->where("car_id=".$res[$i]["id"]." and is_del=0")->sum("mileage");
    ...
    public function byUser(){
        //获取所有单位
        $users=M("User")->where("is_del=0")->select();
