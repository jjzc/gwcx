        ...
        if(empty($_POST)){
            ...
            if($_POST["type"]==1){
                ...
                for($i=0;$i<count($companys)&&count($companys)!=0;$i++){
					...
					$companys[$i]["tingchefei"]=$travelM->where($map)->where(array("state"=>"9"))->sum("parking_rate_sum");
					$companys[$i]["sijizhusubutie"]=$travelM->where($map)->where(array("state"=>"9"))->sum("driver_cost");
					$companys[$i]["chaoshifei"]=$travelM->where($map)->where(array("state"=>"9"))->sum("over_time_cost");
					$companys[$i]["chaogonglifei"]=$travelM->where($map)->where(array("state"=>"9"))->sum("over_mileage_cost");
					$companys[$i]["qitafeiyong"]=$travelM->where($map)->where(array("state"=>"9"))->sum("else_cost");




                    $companys[$i]["qitaCount"]=$travelM->where($map)->where(array("state"=>"9"))->sum("over_time_cost")+$travelM->where($map)->where(array("state"=>"9"))->sum("over_mileage_cost")+$travelM->where($map)->where(array("state"=>"9"))->sum("else_cost");//该单位所有出行费用之和
					...
					$hj["tingchefei"]+=$companys[$i]["tingchefei"];
                    $hj["sijizhusubutie"]+=$companys[$i]["sijizhusubutie"];
