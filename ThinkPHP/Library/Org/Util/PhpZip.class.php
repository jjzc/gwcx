...
class PhpZip
{
    ...
    function ZipDir($dir, $zipfilename, $Todo) {
        ...
        if(is_array($dir)){
            for ($i=0; $i < count($dir); $i++) {
                $res = $this->openFile($dir[$i], $zipfilename , $dir[$i]);
            }
        }else {
            $res = $this->openFile($dir, $zipfilename , $dir);
        }
    ...
    //递归取得下层文件夹下的文件
    function openFile($path, $zipName, $dirname) {
            ...
            While (false !== ($file = readdir($handle))) {
                IF ($file != '.' and $file != '..') {
                    IF (preg_match('/\./', $file . @basename())) {
                        ...
                        fclose($fd);
                        $this->addFile($fileValue, $path_name . '/' . $file);
                    } Else {
                        $this->openFile($path . '/' . $file, $zipName . '/' . $file, $dirname);
                    }
                }
            }
