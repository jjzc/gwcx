$(document).ready(function(e){
	...
	$.extend({
		...
		openWin:function(settings, options){		//打开新窗口
			...
			//修改By MrQ
			win.css("height", $(window).outerHeight()-52).attr('win-height', $(window).outerHeight()-52);
			win.css("width", $(window).outerWidth()-10).attr('win-width', $(window).outerWidth()-10);
			

			//winContent.css({'height':parseInt(win.css('height')) - 25,'width':parseInt(win.css('width')) - 2});	
			//修改By MrQ
			winContent.css({'height':$(window).outerHeight() - 52-26,'width':$(window).outerWidth() - 12});	
			...
			if(propertyString.target == 'iframe'){
				winContent.append('<iframe src="' + settings.attr('path') + '" frameborder="0" height="100%" width="100%" id="dialogFrame" ></iframe>');
			}
		...
		Init:function(){	//初始化桌面UI
			...
			//初始化桌面图标容器高度
			$('.diskicon').css('height',($(window).outerHeight()-60)+'px');
			
			//初始化桌面图标排列
			$.arrayDiskIcon(140, 125);
